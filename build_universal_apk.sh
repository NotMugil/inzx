...
echo -e "\033[0;36m=== Inzx Universal & Standalone APK Builder ===\033[0m\n"
...
if type -p java > /dev/null; then
    ...
    echo -e "\033[0;31m  Install JDK or set JAVA_HOME to your JDK path.\033[0m"
    exit 1
fi
...
if [ -f "$KEYSTORE_PATH" ]; then
    ...
    echo -e "\033[0;33m  Keystore not found at $KEYSTORE_PATH. Generating debug keystore...\033[0m"
...
UNIVERSAL_TARGET="$OUTPUT_DIR/inzx-$VERSION_SAFE-universal.apk"
cp "$OUTPUT_DIR/universal_temp/universal.apk" "$UNIVERSAL_TARGET"
...
BUILD_TOOLS_DIR=$(ls -d $HOME/Android/Sdk/build-tools/* 2>/dev/null | sort -V | tail -n 1)
if [ -z "$BUILD_TOOLS_DIR" ] && [ -n "$ANDROID_HOME" ]; then
...
STRIPPED_APK="$OUTPUT_DIR/inzx-$VERSION_SAFE-unaligned.apk"
FINAL_ARM64_APK="$OUTPUT_DIR/inzx.apk"

STRIPPER_SCRIPT="$REPO_ROOT/tool/strip_apk.py"

python3 "$STRIPPER_SCRIPT" "$UNIVERSAL_TARGET" "$STRIPPED_APK" "arm64-v8a"
